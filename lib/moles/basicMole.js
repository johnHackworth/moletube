...
(function() {
  ...
  mole.prototype = {
    ...
    setAlpha: function() {
      if (this.view.alpha != this.alphaGoal) {
        if (this.view.alpha > this.alphaGoal && this.view.alpha >= 0.1) {
          this.view.alpha = Math.floor(this.view.alpha * 100 - 10) / 100;
        } else if (this.view.alpha < this.alphaGoal && this.view.alpha <= 0.90) {
          this.view.alpha = Math.floor(this.view.alpha * 100 + 10) / 100;
        }
      }
    ...
    walkAround: function() {
      if (this.currentBlock && this.path && this.path.length === 0 && Math.random() > 0.99) {
        this.destination = {
    ...
    animate: function() {
      ...
      if (!this.view.hidden) {
        ...
        if (!this.currentBlock || this.currentBlock.building || this.visibleTileTypes.indexOf(this.currentBlock.type) < 0) {
          this.makeInvisible();
        }
      } else {
        if (this.currentBlock && !this.currentBlock.building && this.visibleTileTypes.indexOf(this.currentBlock.type) >= 0) {
