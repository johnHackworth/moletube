  ...
  city.prototype = {
    ...
    setUndergroundHelp: function() {
      this.undergroundDisplay = new PIXI.Text('Click on empty spaces to add a station (200$) \n' +
        'Select a line and click on stations to create a link', {
          font: "10px Verdana",
          fill: "#FFFFAA",
          strokeThickness: 2,
          stroke: "#333333"
      ...
      this.stage.addVisualEntity(this.undergroundDisplay);
    },
    ...
    createCityBuildings: function() {
      for (var i in this.blocks) {
        if (this.blocks[i].building) {
          this.blocks[i].building.on('click',
            this.showBuildingProfile.bind(this)
          );
        }
      }
    },
    ...
    pathBetweenBlocks: function(blockA, blockB) {
      ...
      var pathToOriginAsCoordinates = this.pathBetweenBlocksAsCoordinates(blockA, origin, 'any');
      for (var i in pathToOriginAsCoordinates) {
        block = this.getBlockFromMap({
          x: pathToOriginAsCoordinates[i][0],
          y: pathToOriginAsCoordinates[i][1]
        });
        path.push(block);
      }
      ...
      // find path between road destination and Point B
      var pathToDestinationAsCoordinates = this.pathBetweenBlocksAsCoordinates(target, blockB, 'any');
      for (i in pathToDestinationAsCoordinates) {
        block = this.getBlockFromMap({
          x: pathToDestinationAsCoordinates[i][0],
          y: pathToDestinationAsCoordinates[i][1]
        });
        path.push(block);
      }
    ...
    getMetroPath: function(blockA, blockB) {
      ...
      for (i in pathFromMetroAsCoordinates) {
        var block = this.getBlockFromMap({
    ...
    constructProject: function(block, type) {
