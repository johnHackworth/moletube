...
(function() {
  ...
  cityBlock.prototype = {
    ...
    destroyTile: function() {
      if (this.tile) {
        this.stage.removeEntity(this.tile);
      }
    },
    refreshTile: function() {
      this.tile.refreshSprite();
    },
    ...
    addDecoration: function() {
      if (this.type === 'c' && Math.random() < 0.05) {
        var options = this.options;
        options.block = this;
        var deco = new window.moletube.models.Tree(options);
        this.decoration.push(deco);
        this.stage.addEntity(deco);
      }
    },
    addStadium: function() {
    ...
    addCustoms: function() {
      this.getCity().customs = this;
      this.building = new moletube.models.Toll({
        x: this.isoX - this.height,
        y: this.isoY - this.width,
        stage: this.stage,
        z: 9999
    ...
    reorderView: function() {
      var prevElement = this.city.getPreviousBuildElement(this);
      if (prevElement && prevElement.building && this.building) {
        this.stage.removeView(this.building.view);
        this.stage.addViewAfter(this.building.view, prevElement.building.view);
      } else if (prevElement && prevElement.decoration.length > 0 && this.building) {
        this.stage.removeView(this.building.view);
        this.stage.addViewAfter(this.building.view, prevElement.decoration[0].view);
      }
      for (var i in this.decoration) {
        // this.decoration[i].reorderView();
      }
    },
    ...
    changeType: function(type) {
      var dummyModel = new type(this);
      for (var i in dummyModel) {
        if (typeof dummyModel[i] === 'function' || !this[i]) {
          this[i] = dummyModel[i];
        }
      }
    },
